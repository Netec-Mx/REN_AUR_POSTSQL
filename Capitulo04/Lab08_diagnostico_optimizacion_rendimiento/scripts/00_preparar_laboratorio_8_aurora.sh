#!/usr/bin/env bash
# ==============================================================================
# Script: 00_preparar_laboratorio_8_aurora.sh
# Propósito: Preparar los prerrequisitos técnicos del Laboratorio 8
#            "Diagnóstico y optimización de consultas" en AWS CloudShell.
#
...
# - El script prepara pg_stat_statements cuando es posible.
# ==============================================================================
...
# Lab 8 trabaja en la raíz actual.
...
# Resumen inicial
# -----------------------------
log "Parámetros de preparación del Laboratorio 8"
...
log "Directorio de trabajo del Laboratorio 8"
pwd
...
# -----------------------------
# Parameter Group para pg_stat_statements
# -----------------------------
...
log "Aplicando parámetros mínimos para diagnóstico SQL"
...
aws rds modify-db-cluster-parameter-group \
  ...
  || warn "No se pudieron aplicar todos los parámetros. Puede ser porque ya existen o requieren permisos."

ok "Parameter Group validado para pg_stat_statements"
...
if psql "$PG_CONN_LAB" -v ON_ERROR_STOP=1 -c "CREATE EXTENSION IF NOT EXISTS pg_stat_statements;" >/dev/null 2>&1; then
  ...
  warn "No se pudo crear pg_stat_statements."
  warn "Puede requerir shared_preload_libraries=pg_stat_statements y reinicio del clúster."
fi

log "Generando evidencia inicial 08_entorno_validacion.txt"

psql "$PG_CONN_LAB" <<'SQL' | tee 08_entorno_validacion.txt >/dev/null
...
cat > "$ENV_FILE" <<EOF
# Variables generadas por 00_preparar_laboratorio_8_aurora.sh
...
export PG_STAT_STATEMENTS_READY='$PG_STAT_READY'
EOF
...
Archivo de variables:   $ENV_FILE
Evidencia inicial:      08_entorno_validacion.txt

Antes de iniciar el Laboratorio 8, ejecuta:
  source ./lab8_aurora_env.sh

Validación rápida:
  psql "host=\$AURORA_ENDPOINT port=\$AURORA_PORT dbname=\$AURORA_DBNAME user=\$AURORA_MASTER_USER password=\$AURORA_MASTER_PASSWORD sslmode=require" -c "SELECT current_database(), current_user, pg_is_in_recovery() AS es_replica;" -c "SELECT extname FROM pg_extension WHERE extname = 'pg_stat_statements';"

Luego continúa con el Reto 1 de la práctica.
EOF

if [[ "$PG_STAT_READY" != "true" ]]; then
  warn "Preparación completada, pero pg_stat_statements no quedó disponible."
  warn "Para el laboratorio 8, valida shared_preload_libraries y reinicia si fuera necesario."
else
