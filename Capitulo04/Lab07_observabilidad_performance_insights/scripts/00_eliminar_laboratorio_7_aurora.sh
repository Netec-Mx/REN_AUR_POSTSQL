#!/usr/bin/env bash
# ==============================================================================
# Script: 00_eliminar_laboratorio_7_aurora.sh
# Propósito: Eliminar los recursos creados para el Laboratorio 7
#            "Observabilidad avanzada con Performance Insights" en Aurora PostgreSQL.
#
...
# - Antes de eliminar infraestructura, intenta limpiar el esquema lab_observability.
# - No crea snapshot final.
# - Está diseñado para ambiente de laboratorio, no producción.
# - Conserva archivos de evidencia 07_* salvo lab7_aurora_env.sh.
# ==============================================================================
...
# -----------------------------
# Detener cargas locales si siguen activas
# -----------------------------
log "Deteniendo procesos locales del Laboratorio 7 si existen"
...
ok "Procesos locales de carga detenidos si existían"

# -----------------------------
# Limpiar esquema temporal si hay conectividad
# -----------------------------
log "Intentando limpiar esquema lab_observability"
...
if [[ -n "$AURORA_ENDPOINT" && "$AURORA_ENDPOINT" != "None" && -n "${AURORA_MASTER_USER:-}" && -n "${AURORA_MASTER_PASSWORD:-}" ]]; then
  if command -v psql >/dev/null 2>&1; then
    ...
    if psql "$PG_CONN_LAB" -v ON_ERROR_STOP=1 -c "SELECT 1;" >/dev/null 2>&1; then
      psql "$PG_CONN_LAB" \
        -v ON_ERROR_STOP=1 \
        -c "DROP SCHEMA IF EXISTS lab_observability CASCADE;" >/dev/null 2>&1 \
        || warn "No se pudo eliminar el esquema lab_observability."

      ok "Esquema lab_observability eliminado o inexistente"
    else
...
warn "Se conservan archivos de evidencia 07_* si existen."
warn "Para eliminarlos manualmente ejecuta: rm -f 07_*"
...
ok "Eliminación del Laboratorio 7 completada"
